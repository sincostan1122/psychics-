...
class AbilityIllusion : Ability<AbilityConceptIllusion>(), Listener {
    ...
    private var arraylocation = Array(concept.illusiontime * 20) { esper.player.location }
   ...
   private fun arrayreload() {
       ...
       player.addPotionEffect(
           PotionEffect(PotionEffectType.INVISIBILITY, 1, 1, false, false, false)
       )
       for (i in 0..concept.durationTime * 20 - 2) {
           arraylocation[i.toInt()] = arraylocation[(i + 1).toInt()]
       }
       arraylocation[(concept.durationTime * 20 - 1).toInt()] = esper.player.location
